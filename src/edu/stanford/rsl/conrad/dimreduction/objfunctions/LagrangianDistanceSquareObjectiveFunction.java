 ...
 */
public class LagrangianDistanceSquareObjectiveFunction extends
	...
	/**
	 * Constructor for the LagrangianDistanceSquareObjectiveFunction
	 */
	public LagrangianDistanceSquareObjectiveFunction() {
		//nothing to do here
	}
	...
	/**
	 * Lagrangian Distance Square Objective Function : L(\vec{x})=\sum_p \sum_q
	 * (||x_p - x_q||_2)^2 \cdot (||x_p - x_q||_2^2 - d_{pq}^2)
	 */
	public double evaluate(double[] x, int block) {
		...
		for (int p = 0; p < distanceMap.length; p++) {
			for (int q = 0; q < distanceMap[0].length; q++) {
				...
				double l = distance * distance;
				// works only with random initialization
				value += Math
						.pow(l * (l - (distanceMap[p][q] * distanceMap[p][q])), 	1);
			}
		}
