...
public class Triangulation extends MultiProjectionFilter {
	...
	protected void processProjectionData(int projectionNumber) throws Exception {
		...
		if(offset > totalProjs/2) throw new Exception("The offset for depth triangulation is larger than the size of the projectionstack!");
...
//		SimpleVector sol = triangulationTrajectory(projectionNumber);
		...
		if(Math.abs(motionfield[projectionNumber])>maxMotion) maxMotion = Math.abs(motionfield[projectionNumber]);
		...
		SimpleVector p1 = new SimpleVector(Configuration.getGlobalConfiguration().getDiaphragmCoordsEntry(projectionNumber));
...
//			motionfield = DoubleArrayUtil.gaussianFilter(motionfield, 10 );
	...
	private SimpleVector iterativeTriangulation(int image1 , int image2, double compensate) {
		...
		while( (Math.abs(1/weight1 - 1/oldWeight1) > 0.1 || Math.abs(1/weight2 - 1/oldWeight2) > 0.1) && iterations <10) {
	...
	private SimpleVector rectifiedIterativeTriangulation(int image1 , int image2, double compensate) {
		...
		SimpleVector c1 = Configuration.getGlobalConfiguration().getGeometry().getProjectionMatrix(image1).computeCameraCenter();		
		SimpleVector c2 = Configuration.getGlobalConfiguration().getGeometry().getProjectionMatrix(image2).computeCameraCenter();

		SimpleVector v1 = c1.clone();
		v1.subtract(c2);

		SimpleVector Rrow = Configuration.getGlobalConfiguration().getGeometry().getProjectionMatrix(image1).getR().getRow(2);
		...
		SimpleMatrix Rect1 = SimpleOperators.multiplyMatrixProd(Pn1.getSubMatrix(0, 0, 3, 3), P1.getSubMatrix(0, 0, 3, 3).inverse(InversionType.INVERT_QR));
		SimpleMatrix Rect2 = SimpleOperators.multiplyMatrixProd(Pn2.getSubMatrix(0, 0, 3, 3), P2.getSubMatrix(0, 0, 3, 3).inverse(InversionType.INVERT_QR));
	...
	private SimpleVector triangulationTrajectory(int projectionNumber) {
		...
		SimpleVector v = geom.getProjectionMatrix(projectionNumber).computeRayDirection(new SimpleVector(Configuration.getGlobalConfiguration().getDiaphragmCoordsEntry(projectionNumber)));
	...
	public void configure() throws Exception {
		...
		setContext(offset+1);
		//setContext(200);
		double corr = (double) Double.parseDouble(JOptionPane.showInputDialog("Enter correction factor diaphragm <-> heart motion:", 1.0 ));
