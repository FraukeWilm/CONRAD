...
public class KPCA extends PCA{
	...
	/**
	 * Uncentered K-matrix corresponding to Schölkopf et al.
	 */
	...
	public void projectTrainingSets(){
		...
		SimpleMatrix k1mpKk1m = SimpleOperators.multiplyMatrixProd(k1mp, SimpleOperators.multiplyMatrixProd(featureMatrix, k1m));
	...
	public double[] projectDataSet(SimpleMatrix shapeMat){
		assert(this.eigenValues != null) : new Exception("Run KPCA first.");
		assert(shapeMat.getCols()*shapeMat.getRows() == numPoints) : new IllegalArgumentException("Input shape does not correspond to training-shapes.");
	...
	/**
	 * This method computes the centered K matrix using the kernel method as described in Schölkopf et al.
	...
	public SimpleMatrix applyWeight(float[] weights){
		throw new UnsupportedOperationException("Method not applicable as variational modes are defined in feature space and not explicitely calculated.");
	}
	...
	public void plotScore(){
		...
		new ImageJ();
		Plot plot = new Plot("Training-data scores: " + kernel.getName(), "1st principal component", "2nd principal component", new double[1], new double[1]);
