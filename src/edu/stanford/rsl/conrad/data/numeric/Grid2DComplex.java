...
public class Grid2DComplex extends Grid2D {
	...
	public float getAtIndex(int i, int j){
		return (float) Math.sqrt(Math.pow(getPixelValue(i*2, j),2) + Math.pow(getPixelValue(i*2+1, j),2) );
	}
	...
	public Grid2D getRealSubGrid(final int startIndexX, final int startIndexY, final int lengthX, final int lengthY){
	...
	public Grid2D getImagSubGrid(final int startIndexX, final int startIndexY, final int lengthX, final int lengthY){
		Grid2D subgrid = new Grid2D(lengthX,lengthY);
		for (int i=0; i < lengthX; ++i){
			for(int j = 0; j < lengthY; ++j){
	...
	public Grid2D getMagnSubGrid(final int startIndexX, final int startIndexY, final int lengthX, final int lengthY){
		Grid2D subgrid = new Grid2D(lengthX,lengthY);
		for (int i=0; i < lengthX; ++i){
			for(int j = 0; j < lengthY; ++j){
	...
	public Grid2D getPhaseSubGrid(final int startIndexX, final int startIndexY, final int lengthX, final int lengthY){
		Grid2D subgrid = new Grid2D(lengthX,lengthY);
		for (int i=0; i < lengthX; ++i){
			for(int j = 0; j < lengthY; ++j){
				float val = (float)Math.atan(getPixelValue((startIndexX+i)*2+1,(startIndexY+j)) 
	...
	public void fftshift() {
                ...
                for(int i = 0; i < dim[1] - halfDim[1]; ++i) {
                        for(int j = dim[0] - halfDim[0]; j < dim[0]; ++j) {
                                this.setRealAtIndex(j, i, copy.getRealAtIndex(j - (dim[0] - halfDim[0]), i + halfDim[1]));
                                this.setImagAtIndex(j, i, copy.getImagAtIndex(j - (dim[0] - halfDim[0]), i + halfDim[1]));
                        }
                }
                // swap section 2 to 3
                for(int i = dim[1] - halfDim[1]; i < dim[1]; ++i) {
                        for(int j = 0; j < dim[0] - halfDim[0]; ++j) {
                                this.setRealAtIndex(j, i, copy.getRealAtIndex(j + halfDim[0], i - (dim[1] - halfDim[1])));
                                this.setImagAtIndex(j, i, copy.getImagAtIndex(j + halfDim[0], i - (dim[1] - halfDim[1])));
                        }
                }
                ...
                for(int i = dim[1] - halfDim[1]; i < dim[1]; ++i) {
                        for(int j = dim[0] - halfDim[0]; j < dim[0]; ++j) {
                                this.setRealAtIndex(j, i, copy.getRealAtIndex(j - (dim[0] - halfDim[0]), i - (dim[1] - halfDim[1])));
                                this.setImagAtIndex(j, i, copy.getImagAtIndex(j - (dim[0] - halfDim[0]), i - (dim[1] - halfDim[1])));
                        }
                }
        }
