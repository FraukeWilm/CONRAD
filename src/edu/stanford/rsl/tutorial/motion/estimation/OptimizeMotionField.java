...
public class OptimizeMotionField {
	...
	public OptimizeMotionField(float[][] motionfield, float [] params, SurfaceBSpline dia, ProjectionLoader pLoad){
		...
		compressionLowerBorder = (int) ((params[4]) * Configuration.getGlobalConfiguration().getGeometry().getReconDimensionZ() / 10.f *spacingZ + oZ);
...
//		splineRender.renderAppendBufferToGrid(dia);
		...
		for(int i = 0; i < motionfield.length; i++) {
			...
			if(Math.abs(evalTop[i]) > 1.0) {
				SimpleVector splinePt3D = triangulationTrajectory(i, new SimpleVector(diaTopX, diaSplineContour[diaTopX],1.0));
				SimpleVector diaOrig = new SimpleVector(diaPositionField[i*3],diaPositionField[i*3+1],diaPositionField[i*3+2]);
		...
		int bestCompression = (int) (params[4]* Configuration.getGlobalConfiguration().getGeometry().getReconDimensionZ() / 10.f *spacingZ + oZ);
		for(int j = 1; j < 10; j++){
			compressionLowerBorder = (int) ((int) (int) (params[4]* Configuration.getGlobalConfiguration().getGeometry().getReconDimensionZ() / 10.f *spacingZ + oZ) +  Configuration.getGlobalConfiguration().getGeometry().getReconDimensionZ() / 20.f *spacingZ *(-sign*j));
			tSpline = createTimeVariantSpline(dia);
			diaRendered = splineRender.SurfaceBSplineRenderingAppendBufferToGrid(tSpline);
			...
			if(absError < avgEval) {
				...
				bestCompression =  (int) ((int) (int) (params[4]* Configuration.getGlobalConfiguration().getGeometry().getReconDimensionZ() / 10.f *spacingZ + oZ) +  Configuration.getGlobalConfiguration().getGeometry().getReconDimensionZ() / 20.f *spacingZ *(-sign*j));
				absError = avgEval;
	...
	public OpenCLCompensatedBackProjectorTPS optimalReconstructor() throws Exception {
		...
		int zPlane = Integer.parseInt(JOptionPane.showInputDialog("Enter z-position of top of the heart in world-coords: ", 0));
	...
	private SimpleVector triangulationTrajectory(int projectionNumber, SimpleVector detectorCoords) {
		...
		SimpleVector pt3D = new SimpleVector(diaPositionField[projectionNumber*3],diaPositionField[projectionNumber*3+1],diaPositionField[projectionNumber*3+2]);
