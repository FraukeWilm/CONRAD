 ...
 *
 */
public class OpenCLCompensatedBackProjector1DCompressionField extends VOIBasedReconstructionFilter implements Runnable, Citeable{
	...
	public OpenCLCompensatedBackProjector1DCompressionField () {
		super();
	}
		...
		if (!initialized) {
			...
			try {
				...
				if (program==null || !program.getContext().equals(this.context)){
					program = context.createProgram(OpenCLCompensatedBackProjector.class.getResourceAsStream("compensatedBackprojectCL1DCompression.cl")).build();
					}
	...
	public void waitForResult(float[][] motionfield) {
	...
	private synchronized void projectSingleProjection(int projectionNumber, int dimz, float [] motionfield){
		...
		CLBuffer<FloatBuffer> motionfieldPtr = context.createFloatBuffer(motionfield.length, Mem.READ_ONLY);
		motionfieldPtr.getBuffer().put(motionfield);
		motionfieldPtr.getBuffer().rewind();
		commandQueue.putWriteBuffer(motionfieldPtr, true).finish();
		// write kernel parameters
		kernelFunction.rewind();
		kernelFunction
		.putArg(volumePointer)
		.putArg(motionfieldPtr)
	...
	public void OpenCLRun(float[][] motionfield) {
		try {
...
//			System.out.println("large Volume " + largeVolumeMode);
			if (largeVolumeMode){
				...
				for (int n =0; n < nSteps; n++){ // For each subvolume
					...
					for (int p = 0; p < maxProjectionNumber; p ++){ // For all projections
						...
						try {
							projectSingleProjection(p, reconDimensionZ, motionfield[p]);
						} catch (Exception e){
	...
	private synchronized void workOnProjectionData(float[][] motionfield){
		if (projectionsAvailable.size() > 0){
			...
			projectSingleProjection(p,  
					getGeometry().getReconDimensionZ(), motionfield[p]);
	...
	public Grid3D reconstructCL(float[][] motionfield){
		init();
