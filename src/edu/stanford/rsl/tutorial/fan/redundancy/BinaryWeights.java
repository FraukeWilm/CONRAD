...
public class BinaryWeights extends Grid2D {
	...
	public BinaryWeights(final double focalLength, final double maxT,
	...
	private void createWeights(Grid2D Weights)
	{
		double lambda, delta;
		...
		for (int t = 0; t < maxTIndex; ++t) {
			...
			for (int b = 0; b < maxLambdaIndex; ++b) {
				...
				// Default weight is 1
				Weights.setAtIndex(maxTIndex - t -1, b, 1.0f);
				...
				{
					Weights.setAtIndex(maxTIndex - t -1, b, 0.f );
				}
			}
		}
	}
	...
	private void createMask(Grid2D Mask)
	{
			// iterate over the projection angles
			for (int b = 0; b < maxLambdaIndex; ++b) {
				// iterate over the detector elements
			...
			for (int b = 0; b < maxLambdaIndex; ++b) {
				...
				if(t < maxTIndex -1)
				{
					...
					{
						Mask.setAtIndex(t, b, 0.f );
						Mask.setAtIndex(t+1, b, 0.f );
					}
				}
			}
		}
