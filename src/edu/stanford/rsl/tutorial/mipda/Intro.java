...
public class Intro {
	...
	public void basicIntro() {
		...
		float [] a = new float [] {3.0f,2.0f,1.0f};
		v1 =  new SimpleVector(3);
		v1.init(a);
		if (v1 != null)
		...
		double [] b = new double [] {Math.random(), Math.random(), Math.random()};
		vRand.init(b);
		...
		SimpleVector first = new SimpleVector (3);
		first.init(1,4,7);
		SimpleVector second = new SimpleVector (3);
		second.init(2,5,8);
		SimpleVector third = new SimpleVector (3);
		third.init(3,6,9);
		
		M.setColValue(0, first);
		M.setColValue(1, second);
		M.setColValue(2, third);
		...
		SimpleVector Vones = new SimpleVector(3);
		Vones.init(1,1,1);
		Mones.setColValue(0, Vones);
		Mones.setColValue(1, Vones);
		Mones.setColValue(2, Vones);
		
		//create a 3x3 matrix Mzeros of zeros (all entries are 0)
		Mzeros = new SimpleMatrix(3,3);
		SimpleVector Vzeros = new SimpleVector(3);
		Vzeros.init(0,0,0);
		Mzeros.setColValue(0,Vzeros);
		Mzeros.setColValue(1, Vzeros);
		Mzeros.setColValue(2, Vzeros);
		

		
		//create a 3x3 identity matrix
		Midentity = new SimpleMatrix(3,3);
		SimpleVector FirstRow = new SimpleVector(3);
		FirstRow.init(1,0,0);
		SimpleVector SecondRow = new SimpleVector(3);
		SecondRow.init(0,1,0);
		SimpleVector ThirdRow = new SimpleVector(3);
		ThirdRow.init(0,0,1);
		Midentity.setColValue(0,FirstRow);
		Midentity.setColValue(1, SecondRow);
		Midentity.setColValue(2, ThirdRow);
		...
		ResMat = SimpleOperators.multiplyMatrices(Mtrans,M);
		if (ResMat != null)
		...
		subVector = colVector.getSubVec(0, 2);
		if (subVector != null)
		...
		if (M != null) {
			...
			double maxM = maxVec.max();  //compute total maximum
			System.out.println("Max(M) = " + maxM);
		}

		//norms
		matrixNormL1 = M.norm(MatrixNormType.MAT_NORM_FROBENIUS); // Frobenius norm of M
		vecNormL2 = colVector.norm(VectorNormType.VEC_NORM_L2); // L2 vector norm of colVector
	...
	public void gridIntro(){
		...
		image = new Grid2D(imageSizeX, imageSizeY);
		...
		for (int j = 0; j < imageSizeY; j++) {
			for (int i = 0; i < imageSizeX; i++) {
				if (Math.sqrt(Math.abs(127-i)*Math.abs(127-i) + Math.abs(127-j)*Math.abs(127-j)) <= radius) {
					image.putPixelValue(i, j, insideVal);
				}
			}
		}
		...
		// finally you need to extract the image with getSubGrid(0)
       	mrImage = ImageUtil.wrapImagePlus(IJ.openImage(filename)).getSubGrid(0);
		if (mrImage != null)
		...
		convolution = conv.convolve(ip, kernel, kw, kh);
		if (ip != null) {
		...
		imageFileName = "mr12out.jpeg"; 
		
		outFileName = outputDataLoc + imageFileName;
		if (mrImage != null) {
			IJ.save(ImageUtil.wrapGrid(convolvedImage, imageFileName),outFileName);  //save the image using IJ and ImageUtil
		}			
	}
